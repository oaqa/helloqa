package edu.cmu.lti.oaqa.openqa.dso.phase.answertype;
...
import edu.cmu.lti.oaqa.openqa.dso.framework.base.AbstractAnswerTypeExtractor;
import edu.cmu.lti.oaqa.openqa.dso.question.ICFeatures;
import edu.cmu.lti.oaqa.openqa.dso.util.LogUtil;
import edu.cmu.lti.util.Pair;

public class AnswerTypeExtractor extends AbstractAnswerTypeExtractor {
	...
	public String extractAnswerTypes(String question) {
		...
		String[] questionSeg = question.replaceAll("\\p{Punct}", " ").trim()
				.split(" ");
		...
		if (questionSeg.length > 0) {
			if (interrogativeSet.contains(questionSeg[0])
					&& questionUnigrams.contains("or")) {
