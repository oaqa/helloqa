...
public class RDFAnswerGenerator {
	...
	private Model loadCachedModel(String eventName) {
		...
		InputStream in = FileManager.get().open(
				getCachedModelFilename(eventName));
	...
	private void saveCachedModel(String eventName, Model model) {
		try {
			model.getWriter("TURTLE").write(model,
					new FileOutputStream(getCachedModelFilename(eventName)),
					null);
	...
	public RDFAnswerGenerator(String sparqlServer, boolean isServer,
		...
		String queryString = "CONSTRUCT { ?o ?p ?s }" + "WHERE {"
				+ "?o ?p ?s ." + "FILTER( ?o = <" + resources[0] + ">" + ")"
				+ "}";
