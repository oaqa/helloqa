...
public class PassageMAPEvalPersistenceProvider extends
		AbstractPassageMAPEvalPersistenceProvider {
	...
	public void insertPartialCounts(final Key key, final int sequenceId,
	...
	@Override
	public Multimap<Key, PassageMAPCounts> retrievePartialCounts(
			final ExperimentKey experiment) {
		String select = getSelectPassageAggregates();
		final Multimap<Key, PassageMAPCounts> counts = LinkedHashMultimap
				.create();
		RowCallbackHandler handler = new RowCallbackHandler() {
			public void processRow(ResultSet rs) throws SQLException {
				Key key = new Key(rs.getString("experimentId"), new Trace(
						rs.getString("traceId")), rs.getInt("stage"));
				PassageMAPCounts cnt = new PassageMAPCounts(
						rs.getFloat("docavep"), rs.getFloat("psgavep"),
	...
	public void insertPartialCounts(final Key key, final String sequenceId,
			final PassageMAPCounts cnt) throws SQLException {
		...
		DataStoreImpl.getInstance().jdbcTemplate()
				.update(insert, new PreparedStatementSetter() {
					public void setValues(PreparedStatement ps)
						...
						ps.setString(2, trace.getTrace());
						ps.setString(3, eName);
						ps.setFloat(4, cnt.getDocavep());
						ps.setFloat(5, cnt.getPsgavep());
						ps.setFloat(6, cnt.getAspavep());
						ps.setFloat(7, cnt.getCount());
